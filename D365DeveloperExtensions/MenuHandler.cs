...
namespace D365DeveloperExtensions
{
    public class MenuHandler : AsyncPackage
    {
        ...
        private static void DisplayCrmIntellisense(object sender, EventArgs eventArgs, DTE dte)
        {
            ...
            bool isEnabled = SharedGlobals.GetGlobal<bool>("UseCrmIntellisense", dte, out bool found);
            if (!found)
            {
        ...
        private static void ToggleCrmIntellisense(object sender, EventArgs e, DTE dte)
        {
            ...
            CrmConnect client = SharedGlobals.GetGlobal<CrmConnect>("CrmService", dte, out found);
            if (!isEnabled) //On
            {
                if (HostWindow.IsCrmDexWindowOpen(dte) && client != null)
                    return;
            }
            else
            {
                ...
                SharedGlobals.SetGlobal<System.Collections.Generic.List<object>>("CrmMetadata", null, dte);
