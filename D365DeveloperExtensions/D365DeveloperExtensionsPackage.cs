...
namespace D365DeveloperExtensions
{
    ...
    [InstalledProductRegistration("#110", "#112", "3.0.21180.1625", IconResourceID = 400)] // Info on this package for Help/About
    ...
    public sealed class D365DeveloperExtensionsPackage : AsyncPackage
    {
        ...
        public void SolutionEventsOnBeforeClosing()
        {
            try
            {
                ...
                SharedGlobals.SetGlobal<bool?>("UseCrmIntellisense", null, dte);

                CrmConnect client = SharedGlobals.GetGlobal<CrmConnect>("CrmService", dte, out bool found);
                if (client == null || !found)
                    SharedGlobals.SetGlobal<CrmConnect>("CrmService", null, dte);

                List<object> meta = SharedGlobals.GetGlobal<List<object>>("CrmMetadata", dte, out found);
                if (meta != null || !found)
                {
                    SharedGlobals.SetGlobal<List<object>>("CrmMetadata", null, dte);
